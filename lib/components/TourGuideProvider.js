...
export const TourGuideProvider = ({ children, wrapperStyle, labels, tooltipComponent, tooltipStyle, androidStatusBarVisible, backdropColor, animationDuration, maskOffset, borderRadius, verticalOffset, startAtMount = false, dismissOnPress = false, }) => {
    ...
    const updateCanStartFlag = useCallback((key) => {
        const newObj = { ...canStartFlagRef.current };
        if ((Array.isArray(stepsRef.current[key]) &&
    ...
    const moveToCurrentStep = useCallback(async (key) => {
        var _a, _b, _c, _d;
        try {
            await ((_b = (_a = currentStep[key]) === null || _a === void 0 ? void 0 : _a.moveIntoView) === null || _b === void 0 ? void 0 : _b.call(_a));
        }
        catch (error) {
            if (__DEV__) {
        ...
        const size = await ((_c = currentStep[key]) === null || _c === void 0 ? void 0 : _c.target.measure());
        if (isNaN(size.width) ||
            isNaN(size.height) ||
            isNaN(size.x) ||
            isNaN(size.y)) {
        ...
        await ((_d = modalRef.current) === null || _d === void 0 ? void 0 : _d.animateMove({
    ...
    const getCurrentStep = useCallback((key) => currentStepRef.current[key], []);
    ...
    return (React.createElement(View, { style: containerStyle },
        React.createElement(TourGuideContext.Provider, { value: ctx },
            children,
            React.createElement(Modal, { ref: modalRef, ...modalProps }))));
};
